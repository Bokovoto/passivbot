...
# Създаване на обект от тип 'l' (signed long) с инициализационни стойности от списък
my_array = np.array([1, 2, 3, 4, 5], dtype=np.int64)



@cuda.jit
def calc_neat_grid_long_kernel(balance, psize, pprice, highest_bid, ema_band_lower, inverse, do_long, qty_step, price_step, min_qty, min_cost, c_mult, grid_span, wallet_exposure_limit, max_n_entry_orders, initial_qty_pct, initial_eprice_ema_dist, eqty_exp_base, eprice_exp_base, auto_unstuck_wallet_exposure_threshold, auto_unstuck_ema_dist, result, cuda=True):
    pos = cuda.grid(1)
    if pos < result.size:
        if wallet_exposure_limit == 0.0:
            result[pos] = (0.0, 0.0, "")
        elif not do_long and psize[pos] == 0.0:
            result[pos] = (0.0, 0.0, "")
        else:
            result[pos] = (entry_qty, entry_price, "long_ientry")

# Създаване на празен масив `result` с подходящ размер
result = np.empty_like(balance)

# Извикване на ядрото с необходимите аргументи и конфигурация на блокове и нишки
calc_neat_grid_long_kernel[blockspergrid, threadsperblock](
    balance, psize, pprice, highest_bid, ema_band_lower, inverse, do_long,
    qty_step, price_step, min_qty, min_cost, c_mult, grid_span, wallet_exposure_limit,
    max_n_entry_orders, initial_qty_pct, initial_eprice_ema_dist, eqty_exp_base,
    eprice_exp_base, auto_unstuck_wallet_exposure_threshold, auto_unstuck_ema_dist,
    result
)
            ientry_price = cuda.fmin(highest_bid[pos], pprice[pos] * (1 - initial_eprice_ema_dist))
            min_ientry_qty = calc_min_entry_qty(ientry_price, inverse, qty_step, min_qty, min_cost)
            if psize[pos] < min_ientry_qty * 0.9:  # initial entry
                ...
                if psize[pos] > 0.0:  # partial initial entry
                    entry_qty = max(min_ientry_qty, cuda.round(entry_qty - psize[pos], qty_step))
                result[pos] = (entry_qty, ientry_price, "long_ientry")
            else:
                wallet_exposure = qty_to_cost(psize[pos], pprice[pos], inverse, c_mult) / balance
                if wallet_exposure >= wallet_exposure_limit * 0.99:
                    ...
                    if auto_unstuck_wallet_exposure_threshold != 0.0:
                        ...
                        if wallet_exposure > threshold * 0.99:
                            auto_unstuck_entry_price = cuda.fmin(highest_bid[pos], pprice[pos] * (1 - auto_unstuck_ema_dist))
                            ...
                            result[pos] = (
                                cuda.fmax(auto_unstuck_qty, min_entry_qty),
                                auto_unstuck_entry_price,
                                "long_unstuck_entry",
                            )
                    else:
                        ...
                        if len(grid) == 0:
                            ...
                            for i in range(len(grid)):
                                if grid[i][1] > pprice[pos] * 0.9995:
                                    continue
                                if grid[i][4] > wallet_exposure_limit * 1.1:
                                    break
                                entry_price = cuda.fmin(highest_bid[pos], grid[i][1])
                                min_entry_qty = calc_min_entry_qty(entry_price, inverse, qty_step, min_qty, min_cost)
                                grid[i][1] = entry_price
                                grid[i][0] = cuda.fmax(min_entry_qty, grid[i][0])
                                comment = "long_primary_rentry"
                                if not entries or (entries[-1][1] != entry_price):
                                    entries.append((grid[i][0], grid[i][1], comment))
                            result[pos] = entries[0] if entries else (0.0, 0.0, "")
    pass



def gpu_function(gpu_id, func, *args):
    cuda.select_device(gpu_id)
    return func(*args)

if __name__ == "__main__":
    processes = []
    functions = [calc_neat_grid_long_kernel]

    for gpu_id in range(cuda.get_device_count()):
        process = multiprocessing.Process(target=gpu_function, args=(gpu_id, functions[0]))
        process.start()
...
def calc_neat_grid_long(
    ...
    result = np.empty_like(balance, dtype=np.object)
    threadsperblock = 256
    blockspergrid = (balance.size + threadsperblock - 1) // threadsperblock
    calc_neat_grid_long_kernel[blockspergrid, threadsperblock](
        balance,
        psize,
        pprice,
...
def approximate_neat_grid_long(
    ...
    # Имплементация на func4 функцията тук
    pass
...
def find_last_entry_qty_long(
    ...
    grid_val0 = eval_neat_entry_grid_long_cuda(
        balance,
        initial_entry_price,
        inverse,
        qty_step,
    ...
    grid_val1 = eval_neat_entry_grid_long_cuda(
        balance,
        initial_entry_price,
        inverse,
        qty_step,
    ...
    result = interpolate_cuda(wallet_exposure_limit, val0, val1, guess0, guess1)
    return round_(result, qty_step)
    # Имплементация на func4 функцията тук
    pass
...
def find_last_entry_qty_short(
    ...
    grid_val0 = eval_neat_entry_grid_short_cuda(
        balance,
        initial_entry_price,
        inverse,
        qty_step,
    ...
    guess1 = guess0 * (1.2 if val0 < wallet_exposure_limit else 0.8)
    grid_val1 = eval_neat_entry_grid_short_cuda(
        balance,
        initial_entry_price,
        inverse,
        qty_step,
...
def calc_whole_neat_entry_grid_long(
    ...
    grid = eval_neat_entry_grid_long_cuda(
        balance,
        initial_entry_price,
        inverse,
        qty_step,
...
def calc_whole_neat_entry_grid_short(
    ...
    grid = eval_neat_entry_grid_short_cuda(
        balance,
        initial_entry_price,
        inverse,
        qty_step,
